	  ...
	  public void paintComponent(Graphics g)
	   {
		  ...
		  try
		  {
		      Thread.sleep(100);  // Wait half a second, for movement
		  }
		...
		if (mysnake.get(bodysize).getX()>1000) //Check to see if it gets stuck in the top wall
       	{
       		 end++;
       	}

		   
		 if (mysnake.get(bodysize).getY()<0)  //Check to see if it gets stuck in the bottom wall
       	{
       			
       		end++;
       	}
		 
		 if (mysnake.get(bodysize).getY()>1000 ) //Check to see if it gets stuck in the right wall
     	{
     		 end++;
     	}
		 

     	if (mysnake.get(bodysize).getX()<0) //Check to see if it gets stuck in the left wall
     	{
     		 end++;
     	}
